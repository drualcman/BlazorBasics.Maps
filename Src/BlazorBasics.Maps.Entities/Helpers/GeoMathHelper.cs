...
{
    public double CalculateRandomAngleInRadians()
    {
    ...
    public double CalculateRandomDegrees() =>
        new Random().Next(0, 360);

    public double ConvertToRadians(double grados) =>
        grados * (Math.PI / 180.0);

    public double ConvertToGrades(double rad) =>
        rad * (180.0 / Math.PI);

    public double CalculateRandomDistance(double distance)
    {
        Random random = new Random();
