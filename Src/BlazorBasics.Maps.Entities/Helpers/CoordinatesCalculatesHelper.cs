namespace BlazorBasics.Maps.Entities.Helpers;
public class CoordinatesCalculatesHelper
{
    ...
    public double GetLatitudeFromDegreesPerMetter(double latitude, double angle, double distanceInMetters)
    {
        double y = Math.Sin(angle) * distanceInMetters;
        return latitude + (y * DEGREES_PER_METTER_OF_LATTITUDE);
    }

    public double GetLongitudeFromDegreesPerMetter(double latitude, double longitude, double angle, double distanceInMetters)
    {
    ...
    public double CalculateDistanceInMetters(ILatLong origin, ILatLong destination)
    {
